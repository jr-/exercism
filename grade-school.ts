...
export class GradeSchool {
    ...
    add(name: string, gradeNum: number) {
        ...
        if (this.students[gradeNum]) {
            let index = 0
            while (index < this.students[gradeNum].length && name > this.students[gradeNum][index]) {
                index++
            }
            this.students[gradeNum].splice(index, 0, name)
        } else {
            this.students[gradeNum] = [name]
        }
    }
  
    grade(gradeNum: number) {
        return this.students[gradeNum] ? [...this.students[gradeNum]] : []
    }
  }
  /*
  Instructions
...
Peter, and Zoe in grade 2 and Jim in grade 5. 
...
your implementation allows outside code to mutate the school's internal DB directly, 
